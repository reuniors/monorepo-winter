import { aS as addDays, b0 as isToday$1, aC as format, aP as startOfMonth, aQ as endOfMonth, b1 as eachDayOfInterval, aT as startOfDay, aB as toZonedTime, az as parseISO, aW as addMinutes, aA as formatInTimeZone, aD as fromZonedTime, e as reactExports, aR as startOfWeek, b2 as endOfWeek, b3 as isSameDay, b4 as subMonths, b5 as addMonths, j as jsxRuntimeExports, b6 as parse, b7 as isWithinInterval, b8 as differenceInMinutes, b9 as isSameMonth, aj as SwiperSlide, M as useTranslation } from "./vendor_react-CZodnfjS.js";
import { f as IonToolbar, d as IonButton, b as IonIcon, ag as calendarOutline, o as IonSpinner, bD as chevronUpOutline, bC as chevronDownOutline, i as IonButtons, av as chevronBackOutline, a4 as chevronForwardOutline } from "./vendor_ionic-DAnbjjdE.js";
import { v as rzrApi, w as TagType, t as transformStandardResponseToCamelCase, N as getPauseBetweenReservations } from "./index-DZE_gcZd.js";
import { v as getAppTimezone, b as useUser, Y as useAppSelector, q as useGetFeWorkersQuery, Z as reservationTimeInterval, _ as getUtcDateFormattedInTz, $ as parseUtcDate, S as SwiperWrapper, k as useContentRefFunctions } from "./App-kfoom-0N.js";
import { u as useGetFeWorkerWorkingShiftsByDaysQuery } from "./workingShift.fe-services-Cjkq9mMg.js";
import { d as useGetFeLocationReservationsQuery } from "./reservation.services-CMRK2_l-.js";
...
function canGapFitServices(gap, totalServiceDuration, pauseBetweenReservations = PAUSE_BETWEEN_SLOTS_MINUTES) {
  return gap.duration >= totalServiceDuration;
}
...
function useBookingCalendar({
  ...
  const today = reactExports.useMemo(
    () => startOfDay(toZonedTime(/* @__PURE__ */ new Date(), timezone)),
    [timezone]
  );
  ...
  const weekStartDate = reactExports.useMemo(() => {
    ...
    const weekStart = startOfWeek(weekFirstDayInAppTz, { weekStartsOn: 1 });
  ...
  const weekEndDate = reactExports.useMemo(() => {
    ...
    const weekEnd = endOfWeek(weekLastDayInAppTz, { weekStartsOn: 1 });
  ...
  const filteredWorkers = reactExports.useMemo(() => {
    ...
    return workersData.data.filter(
      (worker2) => workerIdsToSend.includes(worker2.id)
    );
  ...
  reactExports.useEffect(() => {
    if (gapsData == null ? void 0 : gapsData.data) {
      ...
      datesToProcess.forEach((date) => {
        ...
        const dateKeyWithDashes = formatInTimeZone(
          dateInUtc,
          timezone,
          "yyyy-MM-dd"
        );
        const dateKeyWithoutDashes = formatInTimeZone(
          dateInUtc,
          timezone,
          "yyyyMMdd"
        );
...
function BookingWeekDays({ selectedDate: propSelectedDate } = {}) {
