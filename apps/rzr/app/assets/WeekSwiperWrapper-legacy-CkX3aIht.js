;
(function () {
  System.register(['./vendor_react-legacy-DonFScn3.js', './vendor_ionic-legacy-mZi8EIkK.js', './index-legacy-B6X2T6eC.js', './App-legacy-CxwuunNq.js', './workingShift.fe-services-legacy-BSEOE1rh.js', './reservation.services-legacy-cv8qPPPU.js'], function (exports, module) {
    'use strict';

    var addDays, isToday$1, startOfDay, parseISO, addMinutes, toZonedTime, fromZonedTime, formatInTimeZone, reactExports, startOfMonth, startOfWeek, endOfWeek, format, isSameDay, subMonths, addMonths, endOfMonth, jsxRuntimeExports, parse, isWithinInterval, differenceInMinutes, isSameMonth, SwiperSlide, useTranslation, IonToolbar, IonButton, IonIcon, calendarOutline, IonSpinner, chevronUpOutline, chevronDownOutline, IonButtons, chevronBackOutline, chevronForwardOutline, rzrApi, TagType, transformStandardResponseToCamelCase, getAppTimezone, useGetFeWorkersQuery, reservationTimeInterval, getUtcDateFormattedInTz, parseUtcDate, SwiperWrapper, useContentRefFunctions, useGetFeWorkerWorkingShiftsByDaysQuery, useGetFeLocationReservationsQuery;
    return {
      setters: [module => {
        addDays = module.aT;
        isToday$1 = module.b0;
        ...
        parse = module.b5;
        isWithinInterval = module.b6;
        differenceInMinutes = module.b7;
        isSameMonth = module.b8;
        ...
        reservationTimeInterval = module.W;
        getUtcDateFormattedInTz = module.X;
        parseUtcDate = module.Y;
      ...
      execute: function () {
        ...
        function useBookingCalendar({
          ...
          const weekEndDate = reactExports.useMemo(() => {
            const weekEnd = endOfWeek(currentWeek[6], {
              weekStartsOn: 1
            });
          ...
          }, [gapsData, selectedServices, datesToProcess, workersById, workersData, isCalendarOpen]);
          ...
          reactExports.useEffect(() => {
            ...
            if (!hasSlots) {
              if (isCalendarOpen) {
                ...
                const firstDayInCurrentWeek = currentWeek.find(date => {
                  const dateStr = format(date, "yyyy-MM-dd");
                  return daysWithSlots.has(dateStr);
                });
                if (firstDayInCurrentWeek) {
                  setSelectedDate(firstDayInCurrentWeek);
                  hasInitializedDate.current = true;
                  return;
                }
              }
          ...
          }, [activeSlots, daysWithSlots, currentWeek, currentMonth, isCalendarOpen]);
          reactExports.useEffect(() => {
          ...
          }, [isLoading, isFetching, selectedDate, activeSlots, daysWithSlots, currentWeek, currentMonth, isCalendarOpen]);
          return {
            // State
            currentWeek,
            currentMonth,
        ...
        function ReservationCalendarProvider({
          ...
          }, [timeSlots, daysWithSlots, currentWeek, currentMonth, isCalendarOpen, firstAvailableDate]);
          reactExports.useEffect(() => {
            if (isWorkersWorkingShiftsFetching || isReservationsLoading) return;
            if (selectedDate !== null) return;
            if (!timeSlots || timeSlots.length === 0) return;
            if (daysWithSlots.size === 0) return;
            if (isCalendarOpen) {
          ...
          }, [isWorkersWorkingShiftsFetching, isReservationsLoading, selectedDate, timeSlots, daysWithSlots, currentWeek, currentMonth, isCalendarOpen]);
          const shouldSwitchToNextMonth = reactExports.useCallback((week, selectedDateValue) => {
            if (!selectedDateValue) {
              return false;
            }
      ...
      `,
        ...
        function WeekView({
          ...
          reactExports.useEffect(() => {
            if (swiperRef.current?.swiper && currentWeekIndex >= 0) {
              const currentIndex = swiperRef.current.swiper.activeIndex;
              if (currentIndex !== currentWeekIndex) {
                swiperRef.current.swiper.slideTo(currentWeekIndex, 0);
              }
            }
          }, [currentWeekIndex]);
        ...
        function MonthView({
          ...
          reactExports.useEffect(() => {
            if (monthSwiperRef.current?.swiper && currentMonthIndex >= 0) {
              const currentIndex = monthSwiperRef.current.swiper.activeIndex;
        ...
        function WeekSwiperWrapper({
          ...
          reactExports.useEffect(() => {
            ...
            if (currentIndex !== activeSlideIndex) {
              swiperRef.current.swiper.slideTo(activeSlideIndex, slideToAnimationDuration);
            }
          }, [activeSlideIndex, slides.length, slideToAnimationDuration]);
