...
/**
 * Image Reorder Action (Registry-based)
 * 
 * This action uses the ImageActionRegistry to handle reordering for any registered image type.
 * Uses ReorderDataHelper with from/to pairs for efficient reordering.
 */
class ImageReorderAction extends BaseImageReorderAction
{
    ...
    protected function getAttachmentName(array $attributes): string
    {
        ...
        if (!($attachmentConfig['multi'] ?? false)) {
            throw new BadRequestHttpException("Reorder is only available for multi-image attachments");
        }
