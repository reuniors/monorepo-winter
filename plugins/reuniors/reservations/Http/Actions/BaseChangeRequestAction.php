...
abstract class BaseChangeRequestAction extends BaseAction
{
    ...
    public function rules()
    {
        ...
        $rules['changeRequestDate'] = 'nullable|date|after:today';
    ...
    public function handle(array $attributes = [])
    {
        ...
        if ($changeRequestData && $changeRequestData['is_enabled'] && $changeRequestData['scheduled_date']) {
    ...
    {
        $scheduledDate = $scheduledData['scheduled_date'] ?? null;
        ...
        $existingChangeRequest = ChangeRequest::where('scheduled_date', $scheduledDate)
        ...
        if ($existingChangeRequest) {
            ...
            return $this->updateChangeRequest($existingChangeRequest, $attributes, $scheduledDate);
        }

        // Check limits for new change requests
        $existingCount = ChangeRequest::where('scheduled_date', $scheduledDate)
            ->where('entity_type', $this->getEntityType())
            ->when($entityId, function($query) use ($entityId) {
                return $query->where('entity_id', $entityId);
            })
        ...
        return $this->createChangeRequest($attributes, $scheduledDate);
    }

    protected function createChangeRequest(array $attributes, string $scheduledDate)
    {
    ...
    protected function updateChangeRequest(ChangeRequest $changeRequest, array $attributes, string $scheduledDate)
    {
